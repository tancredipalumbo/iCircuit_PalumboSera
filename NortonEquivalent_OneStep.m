function [isApplicable, Gout, header, solution, results] = ...
    NortonEquivalent_OneStep(G,solutionOptions)
%
% function [isApplicable, Gout, header, solution, results] = ...
%    NortonEquivalent_OneStep(G,solutionOptions)
%
% This function implements the calculation of the Norton equivalent based
% on a single-step determination of equivalent conductance and short-circuit
% current. The initial circuit G must have a single port element. This port
% is replaced with a symbolic voltage test source, the entire circuit
% is converted to symbolic form, and the dual source variable (the current)
...
%% Transform circuit for equivalent conductance and short-circuit current calculations
...
% statement that coincides with the Norton equivalent calculation.
...
[G,header,solution] = replacePortWithTestSource(G,port,'V',0);
...
Gout{1}.output{1}.type = 'current';
Gout{1}.output{1}.unitType = 'I';
Gout{1}.output{1}.label = Gout{1}.edges.elemdata{port}.currentLabel;
...
Gout{1}.edges.elemdata{port}.showI = 1;
